...
void Solution2() {
	//1 410 065 408 - макс размер который выделяется
	// но malloc невозможно уронить неверным запросом на память - он всегда выдаёт всё что попросят, 
	//а падать будет уже при обращении к памяти
